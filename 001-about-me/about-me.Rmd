...
I will additionally analyze my bulk RNA sequencing data using a weighted gene correlation network analysis (WGCNA). Correlation networks are based on correlations between quantitiatve measurements and can be described by an $n$ x $m$ matrix as shown below. 
...
I have experience coding in Linux and running bash scripts. This a script to align sequencing data to the deer mouse genome using STAR. The Wilsterman lab has a server where I do most of my computation work on. 
```c
#!/usr/bin/env bash
...
* I hope to better understand which computational packages to impliment for unique questions and projects.

# Evaluating some R code

Scatter plot of Petal length vs Sepal length from "Iris" dataset.
